...
En esta parte exploramos el funcionamiento del servicio HTTPS dentro de la topología de red. Utilizando Wireshark, se capturan y analizan las peticiones y respuestas entre el cliente y el servidor web, lo que permite observar tráfico tráfico cifrado (HTTPS), así como identificar los puertos y protocolos de transporte utilizados.
\subsection{Acceso al servidor web mediante HTTPS}
En esta captura se observa primero la resolución DNS del dominio web.labredes31.com, donde el servidor retorna la dirección IP 172.20.10.5. A continuación, el cliente establece una conexión TCP con el servidor en el puerto 443 y realiza el handshake TLS, en el cual aparece el mensaje \textbf{Client Hello} con el SNI del dominio y la respuesta \textbf{Server Hello}. Tras este proceso, el tráfico pasa a mostrarse como Application Data cifrado, evidenciando que la comunicación HTTP se encapsula en HTTPS. Finalmente, la sesión se cierra mediante el intercambio de paquetes \textcolor{red}{FIN, ACK} y algunas retransmisiones propias del cierre de la conexión TCP.
