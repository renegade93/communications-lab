// publisher_tcp.c
// Simulates a sports journalist reporting match events over TCP
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stdout, "[%ld] [TCP] Connected to %s:%d\n", now, broker_ip, port);

    srand(time(NULL));

    if (demo) {
        // Player pool and event types
        const char *players[] = {
    ...
    if (close(sock) < 0)
        ...
        fprintf(stdout, "[TCP] Connection closed successfully.\n");
